...
regions = {
    "tube_1": (33, 29, 48, 47),
    "tube_2": (66, 26, 81, 45),
    "tube_3": (101, 26, 116, 45),
    "tube_4": (139, 26, 156, 45),
    "tube_5": (176, 26, 193, 45),
    "tube_6": (212, 26, 229, 45),
    "tube_7": (251, 26, 266, 45),
    "tube_8": (286, 30, 298, 50)
}
...
def detect_test_tube(image):
    ...
    for i, (tube, (x1, y1, x2, y2)) in enumerate(regions.items()):
        ...
        if hue is not None:
            ...
            cv2.putText(image, f"pH: {ph:.0f}" if ph is not None else "", (x1-5, y1 + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
        else:
            cv2.putText(image, "H: ", (x1-5, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
            cv2.putText(image, "pH: ", (x1-5, y1 + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
...
def program_1_at_t1(hue_i, hue_p, hue_n, hue_t_list):
    ...
    for i, hue_t in enumerate(hue_t_list, start=1):
        ...
        result = "Positive" if c_value is not None and c_value > 1.5 else "Negative"
        table_data.append({
            "Tube": f"Tube T{i+3}",
            "Hue Value": hue_t,
            "C Value": c_value,
    ...
    total_result = "Continue reaction" if c1 < 1.5 or c2 < 1.5 else "Good operation"
...
def program_1_at_end(hue_i, hue_p, hue_n, hue_t_list):
    ...
    # Determine the total result based on C1 and C2
    total_result = "Unqualified Operation" if c1 < 1.5 or c2 < 1.5 else "Good operation"
...
def program_2_at_t1(hue_n, hue_t_list):
    ...
    # stop_flag = all(c_value <= 1.5 for c_value in [hue_t / hue_n for hue_t in hue_t_list if hue_n is not None])
    total_result =  "Continue reaction"
...
def program_2_at_end(hue_n, hue_t_list):
    ...
    stop_flag = all(c_value <= 1.5 for c_value in [hue_t / hue_n for hue_t in hue_t_list if hue_n is not None])
    total_result = "Good operation" if stop_flag else "Continue reaction"
        ...
        if capture_counter >= capture_interval_seconds:
            ...
            if program_trigger:
                ...
                if selected_program == 1:
                    ...
                    program_result = {
                        'total_result': "Program 2 is running",
                        'table_data': program_return['table_data']
                    }
